import { RoomState, Player, BidSlab, Team, DEFAULT_BID_SLABS } from "../types";
...
// ============================================================
export function getNextBidAmount(currentBid: number, slabs: BidSlab[] = DEFAULT_BID_SLABS): number {
...
export function validateBid(
  ...
  teamCode: string,
  bidAmount: number,
  slabs: BidSlab[] = DEFAULT_BID_SLABS
): string | null {
  if (state.status !== "live") return "Auction is not live";
  if (!state.currentPlayer) return "No player currently up for bid";
  if (state.currentBidderTeam === teamCode) return "Already the highest bidder";
...
export function advanceToNextPlayer(
  ...
  while (nextSetIndex < sets.length) {
    ...
    const setPlayers = allPlayers.filter(
      (p) => p.set === setName && p.status === "upcoming"
    );
  ...
  if (state.reunsoldPhaseEnabled && !state.reunsoldPhaseActive && state.unsoldPlayers.length > 0) {
    const shuffled = fisherYatesShuffle([...state.unsoldPlayers]);
    const nextPlayer = shuffled.shift()!;
    return {
...
// ============================================================
export function finalizePlayer(
  state: RoomState,
  player: Player,
  soldTo: string | null,
