...
export class RoomActor {
  ...
  // ============================================================
  // Emit custom event to room
  // ============================================================
  public emitToRoom(event: string, data?: any): void {
    this.io.to(this.state.roomCode).emit(event, data);
  }
  ...
  // ============================================================
  handleSubmitPlayingXII(
    socket: Socket,
    sessionId: string,
    teamCode: string,
    playingXI: any[], // PlayingXIPlayer[]
    impactPlayerId: string
  ): void {
    this.enqueue(async () => {
      const team = this.state.teams[teamCode];
      if (!team || team.ownerSessionId !== sessionId) {
        socket.emit("action_rejected", { reason: "You do not own this team" });
        return;
      }
      if (this.state.lockedTeams.includes(teamCode)) {
        socket.emit("action_rejected", { reason: "Team is already locked" });
        return;
      }

      // Validation 1: Structure check
      if (!playingXI || !Array.isArray(playingXI) || playingXI.length !== 11) {
        socket.emit("action_rejected", { reason: "Must select exactly 11 Playing XI" });
        return;
      }
      if (!impactPlayerId) {
        socket.emit("action_rejected", { reason: "Must select exactly 1 Impact Player" });
        return;
      }

      // Validation 2: No duplicates across all 12
      const allIds = [...playingXI.map(p => p.playerId), impactPlayerId];
      const uniqueIds = new Set(allIds);
      if (uniqueIds.size !== 12) {
        socket.emit("action_rejected", { reason: "Duplicate players are not allowed" });
        return;
      }

      // Validation 3: Ownership
      const squadIds = new Set(team.squad.map((p) => p.id));
      const allInSquad = allIds.every((id) => squadIds.has(id));
      if (!allInSquad) {
        socket.emit("action_rejected", { reason: "All selected players must belong to your auction squad" });
        return;
      }
      ...
      for (const p of playingXI) {
        if (p.captain) cCount++;
        if (p.viceCaptain) vcCount++;
        if (p.wicketkeeper) wkCount++;
        if (p.captain && p.viceCaptain) {
          socket.emit("action_rejected", { reason: "Captain cannot also be Vice Captain" });
          return;
        }
      }

      if (cCount !== 1) {
        socket.emit("action_rejected", { reason: "Exactly 1 player must be marked as Captain" });
        return;
      }
      if (vcCount !== 1) {
        socket.emit("action_rejected", { reason: "Exactly 1 player must be marked as Vice Captain" });
        return;
      }
      if (wkCount !== 1) {
        socket.emit("action_rejected", { reason: "Exactly 1 player must be marked as Wicketkeeper" });
        return;
      }

      // We no longer use playerService matching here, as analytics will use getMappedPlayer dynamically.
      // But we can store it directly.
      const mappedPlayingXI = playingXI.map(p => ({
        playerId: p.playerId,
      ...
      this.state = {
        ...this.state,
        playingXIIs: {
          ...this.state.playingXIIs,
          [teamCode]: squadSelection,
        },
        lockedTeams: [...this.state.lockedTeams, teamCode],
      };

      this.log("host_action", `✅ ${team.name} has locked their Playing XII`);
      await this.save();
    });
  }

  handleUnlockPlayingXII(
    socket: Socket,
    sessionId: string,
    teamCode: string
  ): void {
    this.enqueue(async () => {
      ...
      // Remove teamCode from lockedTeams
      const newLockedTeams = this.state.lockedTeams.filter(t => t !== teamCode);
      const newPlayingXIIs = { ...this.state.playingXIIs };
      // Note: we don't necessarily clear playingXIIs completely so it stays saved in UI, just unlock it
      // or we can leave it in playingXIIs but they are not in lockedTeams.

      this.state = {
        ...this.state,
        lockedTeams: newLockedTeams,
      };
      this.log("host_action", `🔓 ${team.name} has unlocked their Playing XII`);
      await this.save();
    });
  }
