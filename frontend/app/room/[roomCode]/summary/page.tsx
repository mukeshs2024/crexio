...
import LockedTeamModal from "@/components/live/LockedTeamModal";
...
function groupByRole(squad: Player[]): Record<Role, Player[]> {
  ...
  for (const p of squad) {
    ...
    if (!groups[role as Role]) {
      groups[role as Role] = [];
    }
    groups[role as Role].push(p);
  }
  return groups;
}
...
export default function SummaryPage({ params }: { params: { roomCode: string } }) {
  ...
  const { roomState, isConnected, myTeamCode, isHost, submitPlayingXII, hostAction, extendTimer, setTimerDuration, kickUser, reassignTeam, editPurse } = useAuctionSocket(roomCode);
  ...
  useEffect(() => {
    // Automatically trigger feedback prompt after a short delay on the summary page
    const timer = setTimeout(() => {
      window.dispatchEvent(new Event("open_feedback_widget"));
    }, 1500);
  ...
  const handleRunAnalytics = async () => {
    ...
    try {
      ...
      // Assuming fast execution for now, redirect immediately
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
          ...
          <div className="flex flex-wrap justify-center gap-8 mt-8">
            {[
              { label: "Locked Teams", value: `${roomState.lockedTeams?.length || 0}/10`, color: "#0066FF" },
        ...
        {myTeamCode && !roomState.lockedTeams?.includes(myTeamCode) && (
          <div className="flex justify-center gap-6 mb-10">
            <button
              onClick={() => router.push(`/room/${roomCode}/team-builder`)}
              className="px-8 py-4 rounded-xl font-black text-xl transition-all shadow-[0_0_20px_rgba(0,102,255,0.4)] hover:shadow-[0_0_30px_rgba(0,102,255,0.6)] hover:-translate-y-1"
        ...
        {isHost && roomState.lockedTeams?.length === 10 && (
          <div className="flex justify-center mb-10">
            <button
              onClick={handleRunAnalytics}
              disabled={isAnalyticsRunning}
              className={`px-12 py-5 rounded-xl font-black text-2xl tracking-widest transition-all uppercase ${
                isAnalyticsRunning
                  ? "bg-gray-700 cursor-not-allowed opacity-70 text-gray-400"
                  : "bg-amber-bid text-black shadow-[0_0_30px_rgba(242,183,5,0.6)] hover:shadow-[0_0_40px_rgba(242,183,5,0.8)] hover:-translate-y-1"
              }`}
            >
              {isAnalyticsRunning ? "⏳ RUNNING ANALYSIS..." : "⚡ RUN ANALYSIS"}
            </button>
          </div>
        )}
        ...
        {/* Team filter tabs */}
        <div className="flex flex-wrap gap-3 justify-center mb-10 border-b border-[rgba(255,255,255,0.05)] pb-6">
          <button
            onClick={() => setActiveTeam(null)}
            className={`px-5 py-2.5 rounded-full text-sm font-black uppercase tracking-widest transition-all ${!activeTeam ? "bg-[#0066FF] text-white shadow-[0_0_15px_rgba(0,102,255,0.4)]" : "bg-[#111111] text-muted hover:text-white border border-[rgba(255,255,255,0.05)]"}`}
          >
            All TEAMS
          </button>
          {teams.map((t) => {
            const isConfirmed = roomState.lockedTeams?.includes(t.code);
            return (
              <button
                key={t.code}
                onClick={() => {
                  if (isConfirmed) setLockedTeamModal(t.code);
                  else setActiveTeam(activeTeam === t.code ? null : t.code);
                }}
                className={`px-4 py-2 rounded-full text-sm font-bold transition-all flex items-center gap-2 ${activeTeam === t.code ? "" : "opacity-80 hover:opacity-100"}`}
                style={{
                  background: activeTeam === t.code ? t.colorHex : "#050505",
                  color: activeTeam === t.code ? "white" : t.colorHex,
                  border: `1px solid ${t.colorHex}`,
                }}
              >
                {t.code}
                {isConfirmed && <span className="text-[12px] text-[#22C55E] ml-1 font-black">✓</span>}
              </button>
            );
          })}
        </div>
