...
export interface UseAuctionSocketReturn {
  ...
  submitPlayingXII: (payload: { playingXI: any[], impactPlayerId: string }, overrideTeamCode?: string) => void;
  unlockPlayingXII: () => void;
}
...
export function useAuctionSocket(roomCode: string): UseAuctionSocketReturn {
  ...
  useEffect(() => {
    ...
    socket.on("analytics_completed", (data: any) => {
      if (data && data.error) {
        alert("Analytics Failed: " + data.error);
      }
      if (window.location.pathname.includes(`/results/${roomCode}`)) {
  ...
  const submitPlayingXII = useCallback((payload: { playingXI: any[], impactPlayerId: string }, overrideTeamCode?: string) => {
    const targetTeam = overrideTeamCode || myTeamCode;
    if (!targetTeam || !socketRef.current) return;
    socketRef.current.emit("submit_playing_xii", { roomCode, teamCode: targetTeam, sessionId, ...payload });
  }, [roomCode, sessionId, myTeamCode]);

  const unlockPlayingXII = useCallback(() => {
    if (!myTeamCode || !socketRef.current) return;
    socketRef.current.emit("unlock_playing_xii", { roomCode, teamCode: myTeamCode, sessionId });
  }, [roomCode, sessionId, myTeamCode]);
  ...
  return {
    ...
    kickUser, reassignTeam, editPurse, clearRejection, sendChat, submitPlayingXII, unlockPlayingXII,
  };
}
