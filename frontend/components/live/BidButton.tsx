...
function BidButtonComponent({
  ...
  return (
    <div className="w-full mt-4 sticky bottom-4 z-50 flex flex-col items-center">
      
      {/* Small Centered Purse display */}
      <div className="mb-2 bg-[#0A0A0A] px-4 py-1.5 rounded-full border border-[rgba(255,255,255,0.05)] shadow-md flex items-center justify-center">
        <span className="text-muted text-[10px] uppercase font-bold tracking-widest mr-2">Your Purse</span>
        <span className="text-white font-bold text-xs tracking-wider">{formatLakhs(myTeam?.purseRemaining ?? 0)}</span>
      </div>

      {/* Dominant Bid Button (80% width, reduced height, neon blue) */}
      <motion.button
        whileTap={!isDisabled ? { scale: 0.96 } : undefined}
        className="w-[80%] rounded-xl font-display font-black text-xl uppercase tracking-wider py-2.5 transition-all flex items-center justify-center relative overflow-hidden"
        style={{
        ...
        disabled={isDisabled}
      >
        {/* Subtle glassmorphism glare */}
        {!isDisabled && <div className="absolute top-0 left-0 w-full h-1/2 bg-white/10 rounded-t-xl" />}
        
        {pendingBid ? (
          <span className="flex items-center gap-2 relative z-10">
            <span className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" />
            <span className="text-base">Bidding...</span>
          </span>
        ) : isLeading ? (
          ...
          <span className="relative z-10 flex items-center gap-2">
            <span>BID</span>
            <span>₹{formatLakhs(actualNextBid)}</span>
          </span>
        )}
      </motion.button>
