      ...
      <AnimatePresence mode="wait">
        {soldAnimation && lastSoldPlayer ? (
          ...
          <motion.div
            ...
            className="w-full rounded-2xl overflow-hidden shadow-[0_10px_40px_rgba(0,0,0,0.5)]"
          ...
          >
            ...
            <div className="p-6 md:p-8 flex flex-col items-center justify-center text-center">
              ...
              <div className="flex items-center justify-center gap-2 mb-3">
                 <span className={`${getRoleBadgeClass(player.role)} text-xs px-2 py-0.5 rounded-full font-bold uppercase tracking-widest`}>
                    {player.role}
                 </span>
                 <span className="text-muted text-xs font-bold uppercase tracking-widest">{player.country}</span>
                 {player.isOverseas && <span className="text-xs" title="Overseas">✈️</span>}
                 {player.isMarquee && <span className="text-accent-blue text-xs" title="Marquee">★</span>}
              </div>
              
              {/* Middle: Player Name */}
              <div className="text-white font-black text-4xl md:text-5xl mb-8 leading-none tracking-tight">
                {player.name}
              </div>
              ...
              <div className="flex items-center justify-center gap-8 md:gap-12 w-full">
                 ...
                 <div className="flex flex-col items-center md:items-end text-center md:text-right">
                   <span className="text-muted text-[10px] md:text-xs uppercase font-bold tracking-widest leading-none mb-2">
                     {currentBid > 0 ? "CURRENT PRICE" : "BASE PRICE"}
                   </span>
                   <span
                     className="font-display font-black text-4xl md:text-5xl leading-none tabular-nums"
                     style={{ color: currentBid > 0 ? "#0066FF" : "#FFFFFF" }}
                   >
                     {currentBid > 0 ? formatLakhs(currentBid) : formatLakhs(player.basePrice)}
                   </span>
                   {/* Separate Prominent Highest Bidder Box */}
                   {bidderTeam && (
                      <div className="flex items-center gap-2 mt-3 bg-[#111111] rounded-full px-4 py-1.5 shadow-[0_4px_10px_rgba(0,0,0,0.5)]">
                        <TeamBadgeSVG team={bidderTeam} size="sm" />
                        <span className="text-xs font-bold text-white uppercase tracking-wider">{bidderTeam.code}</span>
                      </div>
                   )}
                 </div>
                 ...
                 <div
                   ref={timerBoxRef}
                   className="w-16 h-16 md:w-24 md:h-24 rounded-2xl flex flex-col items-center justify-center transition-colors shadow-lg shrink-0"
                   style={getTimerBoxStyle()}
                 >
                   {isPaused ? (
                     <span className="text-2xl md:text-3xl font-black leading-none text-status-warning">⏸</span>
                   ) : (
                     <>
                       <span ref={timerTextRef} className="text-3xl md:text-4xl font-black tabular-nums leading-none">
                         {initSeconds}
                       </span>
                       <span className="text-[8px] md:text-[10px] uppercase font-bold mt-1 tracking-widest opacity-80">sec</span>
                     </>
                   )}
                 </div>
              </div>
